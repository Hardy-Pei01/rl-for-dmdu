...
r_metric = "avg"
utopia = np.array([0, -9, 0, -9])
antiutopia = np.array([-90, -20, -0.1, -11])
mdp = MOMDP(problem, seed=random_seed, dreward=4, utopia=utopia, antiutopia=antiutopia)
robj = 1
...
checkpoint_path = f"./results/rl_dam/4_{method}_{N}_{MAXITER}_0{int(epsilon*10)}_{current_time}"
start = datetime.now()
