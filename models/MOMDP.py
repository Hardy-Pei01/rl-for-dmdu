...
class MOMDP():
    def __init__(self, env, seed, dreward, daction=1, utopia=None, antiutopia=None):
        # if env == "dam":
        ...
        # elif env == "lake":
        #     self.env = Lake1(random_seed=seed, ema=False)
        elif env == "lake_discrete":
            self.env = LakeDiscrete1(random_seed=seed, ema=False)
        # elif env == "lake_uncertain":
        #     self.env = LakeUncertain1(random_seed=seed, ema=False)
        ...
        self.dstate = self.env.observation_space.shape[0]
        self.daction = daction
        self.dreward = self.env.reward_space.shape[0]
        self.utopia = utopia
