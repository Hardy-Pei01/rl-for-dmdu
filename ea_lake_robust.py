from datetime import datetime
from algs.EA_Robust_Trainer import ea_robust_training
from utils.ea.create_models import create_lake_deep_training
from models.ema_lake_uncertain import lake_uncertain_problem

if __name__ == "__main__":
    for r_metric in ["10th"]:
        for random_seed in [
                            # 956500800, 2523200676, 1274569960, 2097286424, 3885705317,
                            # 3186775264, 3690172787, 462638671, 1926216712, 3087161096,
                            # 562732020, 2861224539,
                            1350287007, 674137616, 3624030427,
                            703574460, 1883682950, 617160326, 3668976038, 96930842
                            ]:
            current_time_ea = datetime.now().strftime("-%d-%m-%Y-%H-%M")
            start = datetime.now()
            nfe = 1000000
            ea_robust_training(nfe=nfe, random_seed=random_seed, scenario_num=50,
                               model=create_lake_deep_training,
                               problem=lake_uncertain_problem,
                               scenario_path='./results/lake_scenarios/train_scenarios.csv',
                               restore_path=None, borg=False, n_processes=75,
                               results_path=f'./results/ea_lake_robust_1/{r_metric}_{nfe//10000}_{random_seed}{current_time_ea}',
                               checkpoint_path=f'./tmp/ea_lake_robust_1/{r_metric}_{nfe//10000}_{random_seed}{current_time_ea}',
                               r_metric=r_metric, n_obj=2, dam=False)
            print("Time duration of training: ", datetime.now() - start)
