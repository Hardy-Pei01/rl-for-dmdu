...
def logdet(A=None,op=None):
    ...
    ## argument checking
    assert (isinstance(A, float) and np.asarray(A).ndim == 2 and A.shape[0] == A.shape[1],
            'logdet:invalidarg','A ''should be a square matrix of double or single class.')
    if op is None:
        use_chol = 0
    else:
        assert (op == 'chol', 'The second argument can only be a string ''chol'' if it is specified.')
        use_chol = 1
    
    # computation
    if use_chol:
        v = 2 * np.sum(np.log(np.diag(linalg.cholesky(A))))
    else:
        L, U, P = linalg.lu(A)
        du = np.diag(U)
        c = linalg.det(P) * np.prod(np.sign(du))
