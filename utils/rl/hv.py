...
class HyperVolume:
    ...
    def __init__(self, antiutopia, utopia):
        """Constructor."""
        self.antiutopia = antiutopia
        self.utopia = utopia
        self.dimensions = len(antiutopia)
        self.list = []
    ...
    def compute(self, front):
        ...
        for point in front:
            ...
            if weaklyDominates(self.antiutopia, point):
                relevantPoints.append(point)
        relevantPoints = -normalize_data(relevantPoints, self.antiutopia, self.utopia)
        relevantPoints = relevantPoints.tolist()
        self.preProcess(relevantPoints)
        bounds = [-1.0e308] * self.dimensions
        hyperVolume = self.hvRecursive(self.dimensions - 1, len(relevantPoints), bounds)
        return hyperVolume
    ...
    def preProcess(self, front):
        ...
        nodeList = MultiList(self.dimensions)
        nodes = [MultiList.Node(self.dimensions, point) for point in front]
